...
def parse_code(code_dir):
    """Parse code items from the _code directory."""
    code = []

    if not os.path.exists(code_dir):
        return code

    for code_file in sorted(glob.glob(os.path.join(code_dir, "*.md"))):
        ...
        if front_matter_match:
            ...
            code_entry = {
                "name": front_matter.get("title", ""),
                "category": front_matter.get("collection", "code"),
                "date": front_matter.get("date", ""),
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add code
    cv_json["code"] = parse_code(os.path.join(repo_root, "_code"))
